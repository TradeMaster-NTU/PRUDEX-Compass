...
def create_exp_name_custom(exp_prefix,dataset,uncertain, batch=256, num_ensemble=1, 
                       num_layer=2, layer_size=128, seed=0, confthres=0, ber_mean=-1, policy_lr=7E-4, qr_lr=7E-4,buffer_size=1E4,discount = 0.99,temperature=20):
    """
    Create a semi-unique experiment name that has a timestamp
    :param exp_prefix:
    :param exp_id:
    ...
    if 'conf' in exp_prefix:
        ...
        if 'boot' in exp_prefix:
            ...
            return "%s/en_%d_batch_%d_plr_%s_qlr_%s_layer_%d_%d_buffer_%s_discount_%s_tem_%d_bm_%s_uncertain_%s/seed_%d" % (dataset, num_ensemble,
                                                           batch,policy_lr, qr_lr,layer_size,num_layer,buffer_size,discount,temperature, ber_mean,uncertain,seed)
...
def create_log_dir_custom(
    ...
    exp_name = create_exp_name_custom(
        ...
        num_layer=num_layer,layer_size =layer_size, seed=seed,
        confthres=confthres, ber_mean=ber_mean,policy_lr = policy_lr, uncertain=uncertain,
        qr_lr = qr_lr,buffer_size=buffer_isze,discount=discount,temperature=temperature,dataset=dataset)
...
def setup_logger_custom(
    ...
    if first_time:
        log_dir = create_log_dir_custom(
            ...
            qr_lr=variant['trainer_kwargs']['qf_lr'],
            buffer_isze=variant['buffer_size'],
            discount = variant['trainer_kwargs']['discount'],
            temperature=variant['temperature'],
            seed=variant['seed'],
            dataset=variant['dataset'],
            ber_mean=variant["ber_mean"],
            uncertain=variant["uncertainty"])
